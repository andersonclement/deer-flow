...
\end{tikzpicture}
\fig{Figure — L'aire du grand rectangle ($3\times6=18$) est la somme des deux aires
partielles : $3\times(4+2)=3\times4+3\times2$.}
\end{illus}
...
\node[left] at (-0.2,4.75) {\small $b$};
\end{tikzpicture}
\fig{Figure — Le grand carré de côté $a$ se décompose en $(a-b)^{2}$, deux rectangles
$(a-b)\times b$ et un petit carré $b^{2}$ : on en déduit $(a-b)^{2}=a^{2}-2ab+b^{2}$.}
\end{illus}
...
\end{tikzpicture}
\fig{Figure — Le carré de côté $a$ privé d'un carré de côté $b$ (aire $a^{2}-b^{2}$) se
réarrange en un rectangle de dimensions $(a-b)\times(a+b)$.}
\end{illus}
